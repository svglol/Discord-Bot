...
export = {
	...
	User: {
		...
		totalQuotes : (parent) => { const quotes = parent.quotes; return quotes.length; },
		totalMessages: (parent) => { const messages = parent.messages; return messages.length; },
		totalSoundboards: (parent) => { const soundboards = parent.soundboards; return soundboards.length; },
	...
	Query: {
		...
		bot: (parent, args, { client }) => {
			...
			const bot = { uptime: client.startTime, totalUsers: totalUsers, onlineUsers: onlineUsers, connectedUsers: connectedUsers };
			return bot;
		},
