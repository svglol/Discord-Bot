	...
	async execute(interaction: CommandInteraction) {
		...
		if (voiceChannel == undefined) {
			await interaction.editReply(
				"Must be in a voice channel for this command"
			);
			return;
		}
		...
		if (client.soundManager.resources.length > 0) {
			...
			newResource = {
				...
				voiceChannel: resource.voiceChannel,
				audioResource: null,
				guildId: resource.guildId,
				commandName: resource.commandName,
				title: resource.title,
				url: resource.url,
			};
			if (newResource.commandName != undefined) {
				const soundCommand = await client.db.getSoundCommand(
					newResource.commandName
				);
				newResource.audioResource = createAudioResource(
					createReadStream(soundCommand.file),
				...
				newResource.audioResource = createAudioResource(ytStream, {
		...
		const dennyResource: BotAudioResource = {
			interaction: undefined,
			voiceChannel: voiceChannel2,
			guildId,
			audioResource: createAudioResource(createReadStream("./denny.wav"), {
				inputType: StreamType.Arbitrary,
				inlineVolume: true,
			}),
		...
		const keyboardCheckResource: BotAudioResource = {
			interaction: undefined,
			voiceChannel,
			guildId,
			audioResource: createAudioResource(
				createReadStream("./keyboard_check.wav"),
				{ inputType: StreamType.Arbitrary, inlineVolume: true }
			),
			commandName: undefined,
