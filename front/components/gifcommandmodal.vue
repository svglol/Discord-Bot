<template>
  <div class="modal-card">
    <header class="modal-card-head">
      <p v-if="id == '0'" class="modal-card-title">Add New Text Command</p>
      <p v-if="id !== '0'" class="modal-card-title">Update Text Command</p>
    </header>
    ...
    <footer class="modal-card-foot" style="justify-content: flex-end">
      <button
        ...
        @click="
          ...
          $refs.link.setValidity(true);
        "
      >
        Close
      </button>
      <button v-if="id !== '0'" class="button is-primary" @click="updateCommand">
        Update
      </button>
  ...
  watch: {
    ...
    vCommand: function (newVal) {
      ...
      if (newVal !== "" && newVal !== this.command) {
        let response = "OK";
        this.textCommands.forEach((command) => {
